...
consent, it can install the bot's Teams app into a team via API, so nobody
has to manually click "Add" in the Teams UI for every new team.
"""
import logging
...
def ensure_app_installed(team_id: str, settings: Settings) -> None:
    ...
    identity to have been granted TeamsAppInstallation.ReadWriteForTeam.All
    with admin consent — both one-time setup steps. Failures here are
    logged, not raised, so a transient Graph issue doesn't block delivery
    to a team the bot may already be a member of.
    """
    try:
