...
def _patch_token_validator_for_async_jwks() -> None:
    ...
    A blocking call anywhere freezes the event loop for in-flight requests.
    Patch validate_token to offload just the blocking fetch to a thread
    via asyncio.to_thread, so a slow/stuck JWKS lookup cannot stall the
    entire worker process. Mirrors the original method's logic exactly.
    """
