"""
FastAPI application & Cloud Run entrypoint for GTI Teams Bot.

Served directly by uvicorn — an async-native ASGI server, no WSGI/Cloud
Functions bridging layer. One process, one event loop, for the container's
whole life; FastAPI's own `lifespan` below runs startup/shutdown normally.
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "GTI Teams Bot (Agentic - Local) starting | store=%s gti_url=%s gti_key=%s client_id=%s tenant_id=%s",
