...
async def fetch_thread_messages(
    ...
    client = await graph_client.get_client()
...
def get_session_key(activity, scope: str) -> str:
    ...
    A brand-new top-level channel post has no ";messageid=" segment in its
    own conversation.id at all — only a *reply* to it does, pointing back at
    the root. But the opening post's own `activity.id` IS that root id, the
    same value every later reply's ";messageid=" will reference. Falling
    back to the raw conversation.id for the opening post (as an earlier
    version of this function did) would give it a different key than its
    own replies compute, breaking session continuity starting from the very
    first reply.
    """
    if scope == "channel":
...
def get_channel_id(activity) -> str:
    ...
    channelData.channel isn't always populated on every activity (Teams has
    documented inconsistencies here, notably on some mobile clients) — but a
    channel-scope conversation.id always structurally embeds the channel's
    own thread address before any ";messageid=" suffix, so fall back to that
    instead of losing the channel id — and the session-key stability it
    backs — on messages where channelData.channel is missing.
    """
...
async def get_thread_context(activity, scope: str) -> str:
    ...
    # Reuse get_team_id()/get_channel_id() rather than re-reading
    # channelData inline — this is what actually picks up get_channel_id()'s
    # conversation.id fallback for activities where channelData.channel is
    # missing. Microsoft Graph's /teams/{team-id}/channels/{channel-id}/
    # messages/{id} requires team_id in the URL with no way to derive it
    # from channel_id alone — but if THIS activity's own channelData.team is
    # missing, a team_id seen and cached on an earlier message in the same
    # channel (see session_store.get_team_id_for_channel) still lets this
    # call go through instead of giving up.
    team_id = get_team_id(activity)
    channel_id = get_channel_id(activity)
    if not team_id and channel_id:
        team_id = await get_team_id_for_channel(channel_id) or ""
    thread_id = get_thread_root_id(activity.conversation.id)
