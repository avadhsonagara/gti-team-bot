...
Three download paths, tried in order:
  ...
  2. Bot Framework Connector `content_url` (e.g. inline/pasted images,
     ...
     bot's own token (app/teams/bot_client.get_bot_token).
...
Fully async — httpx, same as this app's other HTTP clients (gti/client.py,
graph/client.py), including the bot's own token needed for the content_url
path.
"""
import base64
import logging
...
async def _download(url: str, headers: Optional[dict[str, str]] = None) -> bytes:
...
async def _download_graph_share(content_url: str) -> bytes:
    ...
    return await _download(url, headers)
...
async def _fetch_graph_message_attachments(activity, scope: str) -> list[dict[str, Any]]:
    ...
    try:
        if scope == "channel":
            ...
            # channelData inline — see app/teams/thread.py's
            # get_thread_context() for the same fix and why it's needed.
        ...
        try:
            if content_type == _FILE_DOWNLOAD_INFO:
                ...
                if not download_url:
                    ...
                    continue
                data = await _download(download_url)
                ...
                data = await _download(attachment.content_url, headers)
            ...
            # Deliberately not logging str(exc) / exc_info here — httpx embeds the
