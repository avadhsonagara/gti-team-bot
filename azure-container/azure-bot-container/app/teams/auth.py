...
def _get_jwks_client() -> jwt.PyJWKClient:
    ...
    if _jwks_client is None:
        # cache_jwk_set=True (PyJWT's default) already caches the whole JWKS
        # document for `lifespan` seconds (default 300), which is what
        # actually avoids hitting the endpoint on every request.
        # cache_keys=True adds a second, smaller LRU cache (by `kid`) on top
        # of that so a lookup within the cache window doesn't re-scan the
        # cached document — a minor addition, not what enables caching itself.
