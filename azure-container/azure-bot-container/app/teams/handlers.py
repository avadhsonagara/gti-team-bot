"""
Teams activity handlers — dispatched by main.py's /api/messages route for
...
Fully async end to end — every I/O call in this pipeline (Graph, GTI,
Table/Blob storage, the Bot Framework Connector) is awaited directly rather
than run in a worker thread.
"""
import logging
...
async def _handle_user_query(
    ...
    try:
        ...
        logger.debug("[EVENT] query=%r attachments=%d", preview, len(attachments))
        ...
        if thread_context:
            logger.debug("[THREAD] Injecting channel thread context into prompt:\n%s", thread_context)
        ...
        if scope == "channel":
            ...
            existing_session_id = await get_session_id(team_id, channel_id, session_key)
        else:
            session_key = ""
            team_id = ""
            channel_id = ""
        ...
        if session_key:
            await set_session_id(team_id, channel_id, session_key, session_id)
