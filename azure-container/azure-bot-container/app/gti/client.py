...
Fully async — httpx.AsyncClient, no microsoft-teams-apps SDK. This is what
lets a container replica hold dozens of multi-minute GTI calls open
concurrently on one process without needing one thread per request.
"""
import asyncio
import logging
import random
...
class GTIAgenticClient:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        """Lazily create the shared AsyncClient (needs a running event loop)."""
        if self._client is None:
            async with self._client_lock:
                if self._client is None:
                    self._client = httpx.AsyncClient(
                        headers={"x-apikey": self.api_key, "User-Agent": "gti-teams-bot-agentic/1.0"},
    ...
    async def _send_request_with_retries(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                response = await client.request(method, url, files=files, data=data)
                ...
                # The request already reached GTI and we were waiting on its
                # response when the client-side read timeout fired — GTI may
                # well have received it and still be processing (these are
                # multi-minute agent calls). Unlike the other transport
                # errors below, retrying here is not safe: create_session/
                # post_session_message expose no idempotency key, so a retry
                # risks spinning up a duplicate session or replaying the same
                # message into the one GTI is already working on. Fail fast
                # instead — one wait of up to `self.timeout`, then a clear
                # "took too long" error rather than silently compounding.
                logger.warning(
                    "[GTI] Read timeout after %.0fs — GTI may still be processing; "
                    "not retrying, to avoid creating a duplicate session/message.",
                    self.timeout,
                )
                raise GTITimeoutError(f"GTI request timed out waiting for a response: {exc}") from exc

            except httpx.TransportError as exc:
                # Every other transport failure (connect/write/pool timeouts,
                # lower-level network errors) happens before or while the
                # request is still being sent, so GTI never saw it — safe to
                # retry. httpx.ReadTimeout (the "GTI received it but is still
                # computing" case) is handled separately above, since that's
                # the one transport error where retrying isn't safe.
    ...
    async def create_session(
        ...
        # keys, hence the list-of-tuples form here — httpx's documented way to
        # send a repeated multipart field, same shape `requests` used). Using
        # indexed keys like "files[0]"/"files[1]" here previously meant the
        # backend never saw them as part of its "files" array at all.
