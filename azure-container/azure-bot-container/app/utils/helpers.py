...
def parse_adaptive_card(raw_text: str | None) -> tuple[Optional[dict], str]:
    ...
    # Strip a code fence anywhere in the text, not just at the very start —
    # models sometimes wrap the JSON in a sentence ("Here's the report:\n```json\n{...}\n```").
    # Uses the first/last ``` as boundaries rather than a brace-matching regex:
    # Adaptive Card JSON is always deeply nested, so a naive non-greedy
    # `\{.*?\}` would stop at the first inner "}" it finds instead of the
    # real outer one.
...
async def deliver_message(
    ...
    async def _deliver(activity: dict) -> None:
    ...
    if card:
        try:
            ...
            await _deliver(_card_activity(card))
            return True
        except Exception as exc:
    ...
    try:
        await _deliver(_text_activity(text))
        return True
        ...
        try:
            await _deliver(_text_activity(notice))
            return True
            ...
            if loading_activity_id and edit_in_place:
                try:
                    ...
                    pass
                try:
                    await ctx.send(_text_activity(notice))
                    return True
