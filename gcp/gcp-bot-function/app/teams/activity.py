...
sends to /api/messages) into a plain object with the attributes the rest
of the app expects.

JSON fields are camelCase; the resulting attributes are snake_case, and
Teams-specific `team`/`channel` info is nested under `channelData.team` /
