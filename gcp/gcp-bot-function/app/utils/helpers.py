...
def parse_adaptive_card(raw_text: str | None) -> tuple[Optional[dict], str]:
    ...
    # enclosing fence: a naive "find the first/last ``` anywhere" approach
    # slices out everything between an embedded pair, corrupting otherwise
    # valid JSON. Only strip a fence, and only a genuinely *leading* one, if
...
def deliver_message(
    ...
    If loading_activity_id is set: edit_in_place=True updates that
    placeholder in place (channel threads, where deleting leaves a visible
    tombstone); edit_in_place=False deletes it first, then sends fresh
    (personal/group chats, where a deleted message leaves no trace).
    """
