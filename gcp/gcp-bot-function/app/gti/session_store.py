...
Uses Firestore's synchronous `Client` (not `AsyncClient`) — a Cloud Run
function invokes one Python function per HTTP request, so there's no shared
event loop for a blocking call here to freeze.
"""
import logging
...
def get_session_id(key: str) -> Optional[str]:
    ...
    try:
        doc = client.collection(settings.firestore_bot_config_collection).document(_doc_id(key)).get()
        if not doc.exists:
            return None
...
def get_team_id(key: str) -> Optional[str]:
...
def set_session_id(key: str, session_id: str, team_id: str | None = None) -> None:
    ...
    try:
        client.collection(settings.firestore_bot_config_collection).document(_doc_id(key)).set({
