...
Plain synchronous `requests` calls — a Cloud Run function invokes one Python
function per HTTP request (see main.py / app/teams/http_adapter.py), so
there's no shared event loop for a blocking call here to freeze.
"""
import logging
import random
import time
