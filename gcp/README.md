...
│     (cron: 0 * * * *) │      (OIDC Auth)       │       (rs-alerts)       │ <─────────── │       Alerts API       │
└─────────────────────────┘                        └───────────┬─────────────┘              └────────────────────────┘
                                                               │ Save Cursor
                                                               ▼
                                          ┌─────────────────────────────────────────┐
...
│   │   ├── gti/client.py         # Client for GTI Agentic Sessions API
│   │   ├── teams/                # Bot Framework auth, handlers, and Adaptive Card builders
│   │   └── utils/                # Message deliverer, card parser, and prompt formatters
│   ├── teams-app-manifest/       # Icons and manifest.json template
│   ├── main.py                   # Functions Framework (Flask) HTTP entrypoint
...
3. **Enterprise Security & Observability**:
   - Fully synchronous Functions Framework (Flask) entrypoint — no async runtime dependency.
