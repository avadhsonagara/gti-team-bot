...
def _post_with_retry(url: str, headers: dict, payload: dict) -> requests.Response:
    """
    POST one Teams activity with retry-with-backoff for transient failures
    (429/5xx/network errors). Any other 4xx (bad credentials, bot not in
    the channel, etc.) is a permanent problem and fails immediately.
    """
