...
# -----------------------------------------------------------------------------
# gti-bot — Cloud Run function (2nd Gen), built from source (same pattern as
# rs-alerts below)
# -----------------------------------------------------------------------------
# main.py is a functions_framework/Flask entrypoint (gti_bot_http), so no
# Dockerfile or manual image build is needed — Cloud Build handles it,
# redeploying whenever the uploaded source object's name (which embeds the
# source zip's md5) changes.
resource "google_cloudfunctions2_function" "gti_bot" {
  ...
  service_config {
    ...
    environment_variables = {
      # Matches max_instance_request_concurrency above — the number of
      # concurrent requests one instance should be prepared to serve.
      THREADS            = tostring(var.concurrency)
...
# /api/messages, not IAM (contrast rs_alerts_invoker below, which restricts
# rs-alerts to the scheduler service account).
resource "google_cloud_run_v2_service_iam_member" "gti_bot_public" {
  project  = var.project_id
  location = var.region
  name     = google_cloudfunctions2_function.gti_bot.name
...
resource "google_cloudfunctions2_function" "rs_alerts" {
  ...
  lifecycle {
    precondition {
      condition     = var.rs_alerts_teams_channel_id_or_link != ""
      error_message = "rs_alerts_teams_channel_id_or_link must be set when enable_rs_alerts = true."
    }
    precondition {
      condition     = var.rsa_gti_project != ""
      error_message = "rsa_gti_project must be set when enable_rs_alerts = true."
    }
  }
...
resource "google_cloud_run_v2_service_iam_member" "rs_alerts_invoker" {
  count    = var.enable_rs_alerts ? 1 : 0
  project  = var.project_id
  location = var.region
  name     = google_cloudfunctions2_function.rs_alerts[0].name
  role     = "roles/run.invoker"
