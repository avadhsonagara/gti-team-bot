# GTI Teams Bot (Agentic) — Asynchronous Queue Architecture — PERFORMANCE TEST BUILD

> **This is a load/performance-testing clone of [`azure-queue`](../azure-queue),
> not the production bot.** The one deliberate difference:
> `bot-worker-function/app/gti/client.py` makes **no real call to the Google
> Threat Intelligence API at all** — every query gets a fixed, canned
> response after a simulated `gti_timeout_seconds` (180s by default) delay,
> regardless of what was asked. Everything else — ingest, the Storage Queue
> hand-off, session/output-format persistence, delivery, Adaptive Card
> rendering, error handling — is unchanged, so this exists purely to
> load-test the queue architecture itself (ingest -> Storage Queue -> worker
> -> delivery) under many concurrent/queued jobs with a known, fixed
> per-job latency, without GTI's real latency, rate limits, or API cost.
>
> Resource names default to `gti-teams-bot-ingest-test`/
> `gti-teams-bot-worker-test` (not `azure-queue`'s `gti-teams-bot-ingest`/
> `-worker`) specifically so deploying this can never collide with or
> overwrite the real production deployment.
