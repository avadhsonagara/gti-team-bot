"""
Minimal ActivityContext (ctx) for an inbound message, backed by the Bot
Framework Connector API calls in app/teams/bot_client.py. Provides only
ctx.activity, ctx.send(), and ctx.api.conversations.activities(id).update()/
.delete() — the full surface app/teams/handlers.py and app/utils/helpers.py need.
"""
