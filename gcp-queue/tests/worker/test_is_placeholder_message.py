"""
Automates the five is_placeholder_message() scenarios documented in
../../README.md's "A real bug this design fixes" section — locking in that
behavior with a real regression test instead of a one-time manual check.
"""
...
def test_human_quoting_placeholder_text_verbatim_not_excluded():
    """The whole reason this checks (text AND identity), not text alone — see the README."""
...
def test_placeholder_text_inside_adaptive_card_attachment_also_matched():
    """extract_attachment_text() fallback — a card-only message has empty body.content."""
    msg = {
        "from": {"application": {"id": _BOT_APP_ID}},
        "body": {"content": ""},
        "attachments": [{
            "contentType": "application/vnd.microsoft.card.adaptive",
            "content": '{"type": "AdaptiveCard", "body": [{"type": "TextBlock", "text": "' + PLACEHOLDER_TEXT + '"}]}',
        }],
    }
    assert is_placeholder_message(msg, _BOT_APP_ID) is True
